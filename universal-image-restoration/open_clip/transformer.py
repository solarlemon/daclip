...
class ControlTransformer(nn.Module):
    """
    将一个预先定义好的transformer网络进行封装
    以便在输入的控制向量control的作用下，对输入的序列x进行处理
    """
    ...
    def __init__(self, transformer):
        ...
        # 包含layers个nn.Linear层，将输入的序列x的权重清零
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None,
        ...
        """
        :param x: 输入的序列，形状为(batch_size, sequence_length, hidden_size)的张量
        :param attn_mask:  可选的注意力掩码张量，形状为(batch_size, sequence_length, sequence_length)
        :param output_hiddens:  是否输出隐藏层表示，默认值为False
        :param control: 可选的控制向量，形状为(batch_size, sequence_length, hidden_size)
        :return: 根据output_hiddens参数的值，返回输出结果x和hiddens列表，或者只返回输出结果x
        """
        if output_hiddens:
            hiddens = []  # 存储隐藏层
        for z, r in zip(self.zero_modules, self.transformer.resblocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                # 对残差块进行梯度检查点，并将输出结果x传递给下一个残差块
                # TODO: handle kwargs https://github.com/pytorch/pytorch/issues/79887#issuecomment-1161758372
                x = checkpoint(r, x, None, None, attn_mask)
            else:
                # 直接调用残差块的forward方法，并将输出结果x传递给下一个残差块
                x = r(x, attn_mask=attn_mask)
            zx = z(x)

            if output_hiddens:
                hiddens.append(zx)  # 对输出结果x进行权重清零操作，并将结果传递给hiddens列表

            if control is not None:
                x += control.pop()  # 将control向量中的最后一个元素添加到输出结果x中
